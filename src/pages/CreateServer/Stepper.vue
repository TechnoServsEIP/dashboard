    ...
    >
      ...
      <q-form>
        ...
        >
          ...
          >
            <div class="row">
              <div v-for="game in gamesList" :key="game.name" class="col-3 q-pr-md q-pb-md">
                <q-card>
          ...
          >
                ...
                <div class="col-md-3">
                    <q-card class="my-card">
                      <q-card-section class="bg-grey-1">
                        ...
                        <div class="text-overline price">{{ offersList.dedicated_ip_address.monthly_price }}€/mois</div>
                        <!-- <div class="text-overline price">{{ offer.info.dedicated_ip_address.hourly_price }}€/heure</div> -->
                      </q-card-section>

                      <q-card-section>
                        <div class="text-subtitle2">RAM: {{convertRam(offersList.ram_memory)}}GB</div>
                      </q-card-section>
           ...
           <q-step
            ...
            icon="create_new_folder"
          >
            <p><span style="font-weight: bold;">Instance name: </span>{{this.instanceName}}</p>
            <p><span style="font-weight: bold;">Game: </span>Minecraft</p>
            <p><span style="font-weight: bold;">Offer name: </span>{{this.offersList.name}}</p>
          </q-step>
...
export default {
  ...
  methods: {
    ...
    getOffers() {
      ...
      this.axios.get('/offers/list').then(response => {
    ...
    createServer() {
      ...
      this.axios.post('/docker/create', {
        game: "minecraft", 
        user_id: this.$store.getters['client']._user.ID.toString(),
        server_name: this.instanceName,
      }, {headers: {'Authorization': `${this.$store.getters['client']._user.token}`}}).then(response => {
